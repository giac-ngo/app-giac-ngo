...
import { ChevronDownIcon, CopyIcon, SpeakerWaveIcon, ThumbsUpIcon, ThumbsDownIcon, ChevronDoubleRightIcon, BrainwaveIcon, CommentIcon, DownloadIcon } from '../components/Icons';
...
export const ChatPage: React.FC<{
        ...
        try {
            ...
            liveSessionPromise.current = ai.live.connect({
                ...
                config: {
                    ...
                    systemInstruction: [currentAiConfig.trainingContent, currentAiConfig.additionalTrainingContent].filter(Boolean).join('\n\n'),
                },
            });
        } catch (error: any) {
  ...
  const handleFeedback = (messageId: string | number, feedback: 'liked' | 'disliked') => setFeedbackStatus(prev => ({ ...prev, [messageId]: prev[messageId] === feedback ? undefined : feedback }));
                ...
                <>
                    <header className="chat-main-header">
                        <div className="flex items-center gap-3">
                            {currentAiConfig?.avatarUrl && <img src={currentAiConfig.avatarUrl} alt={currentAiConfig.name} className="w-10 h-10 rounded-full object-cover"/>}
                            <div className="relative" ref={aiSelectorRef}>
                                <button onClick={() => setIsAiSelectorOpen(prev => !prev)} className="flex items-center gap-2 text-lg font-semibold">
                                ...
                                {isAiSelectorOpen && (
                                    <div className="absolute top-full left-0 mt-2 w-72 bg-background-panel border border-border-color rounded-lg shadow-lg z-10">
                                                ...
                                                <li key={ai.id}>
                                                    ...
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-text-light truncate">{ai.description}</p>
                                                        </div>
                                                    </button>
                                                </li>
                                            ))}
                        ...
                        <div className="max-w-4xl mx-auto space-y-2">
                            ...
                            <form onSubmit={handleSendMessage} className="relative">
                                ...
                                <div className="chat-input-wrapper">
                                    ...
                                    <button onClick={handleToggleLiveChat} type="button" title={t.liveChat} disabled={isChatDisabled && !isLiveActive} className={`chat-input-icon-btn chat-live-btn ${isLiveActive ? 'text-accent-red animate-pulse' : ''}`}><BrainwaveIcon className="w-6 h-6"/></button>
