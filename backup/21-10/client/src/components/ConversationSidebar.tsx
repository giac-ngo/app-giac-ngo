...
import { InfoIcon, HelmetIcon, PagodaIcon, LanguageIcon, SignalIcon, CryptoIcon, UserIcon, LogoutIcon, ChatBubbleIcon, LoginIcon } from './Icons';
...
const ConversationSidebar: React.FC<ConversationSidebarProps> = (props) => {
    ...
    return (
        <aside className={`conversation-sidebar relative ${isSidebarCollapsed ? 'conversation-sidebar-collapsed' : 'w-80'} bg-background-panel border-r border-border-color flex flex-col h-screen flex-shrink-0`}>
            ...
            <div className="flex-shrink-0">
                <div className="quick-actions-container">
                    <div className="quick-actions-grid">
                        <button onClick={() => setViewMode('chat')} className={`quick-action-btn ${viewMode === 'chat' ? 'active' : ''}`} title={t.chatMode}><InfoIcon /></button>
                        <button onClick={() => setViewMode('meditation')} className={`quick-action-btn ${viewMode === 'meditation' ? 'active' : ''}`} title={t.meditationMode}><SignalIcon /></button>
                        <button onClick={() => handleQuickAction(t.character)} className="quick-action-btn"><HelmetIcon /></button>
                        <button onClick={() => handleQuickAction(t.context)} className="quick-action-btn"><PagodaIcon /></button>
                    </div>
                    {viewMode === 'chat' && (
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-6">
                {!isSidebarCollapsed && (
                    viewMode === 'chat' ? (
                        <div>
                            <h3 className="text-xs font-semibold text-text-light uppercase tracking-wider mb-3">{t.recentChats}</h3>
                            {!user ? (
                                ...
                                <div className="space-y-1 history-list-container">
                                    {conversations.map(conv => (
                                        <div key={conv.id} className={`group relative rounded-lg ${selectedConversationId === conv.id ? 'history-item-selected' : ''}`}>
                                            <button onClick={() => onSelectConversation(conv)} className="w-full text-left history-item flex items-center gap-3">
                ...
                {user ? (
                    isSidebarCollapsed ? (
                        ...
                        <div className="user-info-card">
                            ...
                            <div className="user-info-actions">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <button onClick={onLogout} className="btn-logout col-span-full">
                                        <LogoutIcon className="w-5 h-5"/>
                                        <span>{t.logout}</span>
                                    </button>
                                </div>
                     ...
                     isSidebarCollapsed ? (
                        ...
                        <button onClick={onGoToLogin} className="w-full py-2.5 bg-primary text-text-on-primary rounded-md font-semibold text-base hover:bg-primary-hover">{t.login}</button>
                   )
                )}
            </footer>
        </aside>
