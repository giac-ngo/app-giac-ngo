...
const Settings: React.FC<SettingsProps> = ({ user, language, systemConfig, onSystemConfigUpdate, onUserUpdate }) => {
    ...
    const handleSavePersonalKeys = async () => {
        ...
        try {
            ...
            onUserUpdate(savedUser); // Update the user in the global state
    ...
    const canViewSystem = user.isAdmin || user.permissions?.includes('settings');
    const canViewPersonal = user.permissions?.includes('settings');
    ...
    return (
        <div className="p-8">
            ...
            <div className="mb-6 border-b border-border-color">
                <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    {canViewSystem && (
                     ...
                     {/* All logged-in users can see their own key settings */}
                    <button
            ...
            {activeTab === 'system' && canViewSystem && localSystemConfig && (
