...
interface PricingCardProps {
    ...
    onCryptoPurchase: (plan: PricingPlan) => void;
    isPurchasing: boolean;
}

const PricingCard: React.FC<PricingCardProps> = ({ plan, user, onPurchase, onCryptoPurchase, isPurchasing }) => {
            ...
            <ul className="space-y-2 text-text-light flex-grow">
                {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start">
                        <svg className="w-5 h-5 mr-2 text-accent-green flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                        <span>{feature}</span>
                    </li>
                ))}
            </ul>
