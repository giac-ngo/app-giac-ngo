...
const apiService = {
    ...
    getTrainingDataForAI: (aiId: number): Promise<TrainingDataSource[]> => {
        return fetch(`/api/ai/${aiId}/training-data`).then(handleResponse);
    },
    ...
    // Send train request to server for a specific AI
    trainAi: (aiId: number, content: string): Promise<void> => {
        return fetch(`/api/ai/${aiId}/train`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
