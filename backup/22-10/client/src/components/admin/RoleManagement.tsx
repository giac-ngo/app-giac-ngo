...
const translations = {
    vi: {
        ...
        permissionLabels: {
            ...
            'pricing': 'Quản lý Giá',
            'user-billing': 'Giao dịch & Nạp Coin (cá nhân)',
            'manual-billing': 'Thanh toán thủ công',
            'templates': 'Giao diện',
    ...
    en: {
        ...
        permissionLabels: {
            ...
            'user-billing': 'Transactions & Top-up (personal)',
...
const ALL_PERMISSIONS: (keyof typeof translations['vi']['permissionLabels'])[] = [
    'dashboard', 'ai', 'users', 'conversations', 'pricing', 'user-billing', 'manual-billing', 'templates', 'finetune', 'settings'
];
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4 border border-border-color rounded-md bg-white">
                                {ALL_PERMISSIONS.map(permissionKey => (
                                    <label key={permissionKey} className="flex items-center space-x-2">
                                        ...
                                        />
                                        <span className="text-sm">{t.permissionLabels[permissionKey]}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
