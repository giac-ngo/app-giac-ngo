...
import { User, AIConfig, SystemConfig, Conversation, Message, ModelType, PricingPlan, Transaction } from '../types';
...
const apiService = {
    ...
    getAiConfigs: (user: User | null): Promise<AIConfig[]> => {
        if (user?.isAdmin) {
            return fetch('/api/ai-configs/all').then(handleResponse);
        }
    ...
    createAiConfig: (aiConfig: Partial<AIConfig>): Promise<AIConfig> => {
        return fetch('/api/ai-configs/create', { // Updated endpoint
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    // ADD: New function to get transactions for a specific user.
