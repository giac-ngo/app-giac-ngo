...
const UserManagement: React.FC<{ user: User, language: 'vi' | 'en' }> = ({ user: adminUser, language }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            let savedUser;
            if (formState.id === 'new') {
                 const { id, ...createPayload } = formState;
                 savedUser = await apiService.createUser(createPayload);
            } else {
                savedUser = await apiService.updateUser(formState as User);
            }
                    ...
                    <div className="space-y-6 pb-20">
                         <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <button className="text-sm text-primary hover:underline">{t.changeAvatar}</button>
                            </div>
                        </div>
                        ...
                        <div className="flex space-x-8">
                             <div className="flex items-center"><input id="isAdmin" type="checkbox" checked={formState.isAdmin || false} onChange={e => handleFormChange('isAdmin', e.target.checked)} className="h-4 w-4 text-primary border-border-color rounded focus:ring-primary" /><label htmlFor="isAdmin" className="ml-2 block text-sm font-medium text-text-main">{t.isAdminLabel}</label></div>
