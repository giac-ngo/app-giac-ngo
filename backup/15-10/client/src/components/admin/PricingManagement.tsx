...
const PricingManagement: React.FC<PricingManagementProps> = ({ language }) => {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            // Ensure numbers are correct and duration can be null
            const planToSave = {
                ...editingPlan,
            ...
            if (editingPlan.id === 'new') {
                ...
                await apiService.createPricingPlan(newPlan as any);
            } else {
                 ...
                 <div className="bg-white shadow-md rounded-lg overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.planName}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.price}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.coinCost}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.duration}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.isActive}</th>
