...
const AiManagement: React.FC<{ language: 'vi' | 'en', user: User }> = ({ language, user }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Admins see all AIs for management purposes
                const [configs, users, sysConfig] = await Promise.all([
                    apiService.getAiConfigs({ ...user, isAdmin: true }), // Fetch all as admin
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Array.from(e.target.files).forEach(file => formData.append('trainingFiles', file));
        setIsUploading(true);
        try {
    ...
    const handleToggleRecording = () => { /* ... existing implementation ... */ };
        ...
        <div className="flex h-full">
            ...
            {selectedAi ? (
                <div className="flex-1 flex">
                    ...
                    <div className="w-2/5 flex flex-col bg-background-light">
                        <h3 className="p-4 border-b border-border-color font-semibold text-center h-[73px] flex items-center justify-center">{t.testChat}</h3>
                        <div className="flex-1 p-4 overflow-y-auto space-y-4">
                            {messages.map(msg => <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : ''}`}><div className={`max-w-md px-4 py-2 rounded-xl ${msg.sender === 'user' ? 'bg-primary text-text-on-primary' : 'bg-background-panel shadow-sm'}`}>{msg.text}</div></div>)}
                        ...
                        <div className="p-4 bg-background-panel border-t border-border-color">
