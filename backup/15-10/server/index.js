...
app.post('/api/ai-configs', async (req, res) => {
    ...
    try {
        let dbUser = null;
        if (user && user.id) {
             // Subscription status check is now done at login, so we can trust the user object from the client for this call
...
app.get('/api/ai-configs/all', async (req, res) => {
    try {
        const configs = await db.getAllAiConfigs();
        res.json(configs);
    } catch (error) {
        res.status(500).json({ message: 'Không thể tải toàn bộ danh sách AI.' });
    }
});
...
app.get('/api/conversations', async (req, res) => {
    ...
    if (!userId) {
        // This case should not happen if the user is logged in, but as a fallback.
        return res.json([]);
    }
    try {
...
app.post('/api/subscriptions/purchase', async (req, res) => {
    ...
    try {
        // In a real app, userId should come from a secure session/token, not the body.
...
const MOCK_CRYPTO_RATES = {
    USDT: 0.1, // $0.1 per coin
    USDC: 0.1, // $0.1 per coin
    ETH: 0.00003, // 0.00003 ETH per coin
};
const MOCK_CRYPTO_ADDRESS = '0x1234567890123456789012345678901234567890';
let mockCryptoTransactions = {}; // In-memory store for mock transactions
...
app.post('/api/crypto/confirm', async (req, res) => {
    ...
    try {
        // In a real app, this would be triggered by a webhook after blockchain confirmation
        // Here we just add the coins directly
        const updatedUser = await db.addCoinsToUser(userId, tx.coins, null, 'crypto');
        delete mockCryptoTransactions[transactionId]; // Clear mock transaction
        res.json(mapAndSanitizeUser(updatedUser));
    } catch(error) {
