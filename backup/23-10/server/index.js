...
app.post('/api/ai-configs/translate', async (req, res) => {
    const { aiConfig, targetLanguage, user } = req.body;
    if (!aiConfig || !targetLanguage || !user || !user.id) {
    ...
    try {
        ...
        const provider = aiConfig.modelType;
        const apiKey = dbUser.apiKeys?.[provider];

        if (!apiKey) {
            return res.status(400).json({ message: `Bạn chưa thiết lập API key cho ${provider.toUpperCase()} trong Cài đặt.` });
        }

        let translatedConfig;
        if (provider === 'gpt') {
            translatedConfig = await gptService.translateConfig(aiConfig, targetLanguage, apiKey);
        } else if (provider === 'gemini') {
            translatedConfig = await geminiService.translateConfig(aiConfig, targetLanguage, apiKey);
        } else {
            // For other providers, just return the original config for now
            translatedConfig = {
                name: aiConfig.name,
        ...
        res.json(translatedConfig);

    } catch (error) {
        console.error('Translation error on server:', error);
        res.status(500).json({ message: error.message || 'Failed to translate AI config.' });
    }
});
...
app.post('/api/ai-configs/:id/latest-conversation', async (req, res) => {
    try {
        ...
        const conversation = await db.getLatestConversationByAiId(req.params.id, user.id);
...
app.post('/api/conversations', async (req, res) => {
    const { aiConfigId, messages, user } = req.body;
    if (!aiConfigId || !messages || !user) {
...
app.post('/api/chat/stream', async (req, res) => {
    ...
    try {
        ...
        if (currentUser && currentUser.id) {
            ...
            currentUser = await db.checkSubscriptionStatus(currentUser.id);
            if (!currentUser) {
...
app.post('/api/users/:id/regenerate-token', async (req, res) => {
    try {
        const updatedUser = await db.regenerateApiToken(req.params.id);
        res.json(mapAndSanitizeUser(updatedUser));
    } catch (error) {
