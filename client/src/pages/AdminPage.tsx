...
type AdminTab = 'dashboard' | 'ai' | 'users' | 'roles' | 'settings' | 'conversations' | 'pricing' | 'manual-billing' | 'templates' | 'finetune' | 'user-billing' | 'files' | 'comments' | 'spaces' | 'dharma-talks' | 'meditation' | 'space-billing';
...
const getFirstAllowedTab = (user: User): AdminTab => {
  ...
  const allowedTabs: AdminTab[] = [
    ...
    'templates', 'finetune', 'settings', 'roles', 'files', 'comments', 'spaces', 'dharma-talks', 'meditation'
  ];
...
const AdminPage: React.FC<AdminPageProps> = ({ user, onLogout, language, setLanguage, systemConfig, onSystemConfigUpdate, onUserUpdate }) => {
  ...
  useEffect(() => {
    ...
    apiService.getSpaceBySlug(effectiveSpaceSlug)
  ...
  const effectiveSpaceSlug = spaceSlug || 'giac-ngo';
  const logoUrl = currentSpace?.imageUrl || (systemConfig ? systemConfig.templateSettings[user.template || systemConfig.template].logoUrl : '');
      ...
      <aside className={`bg-background-panel border-r border-border-color flex flex-col transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        <div className="h-[73px] flex items-center justify-center relative border-b border-border-color px-4 flex-shrink-0">
          {!isSidebarCollapsed && logoUrl && (
            <Link to={`/${effectiveSpaceSlug}/admin/dashboard`} className="flex items-center">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          {hasPermission('meditation') && <NavItem tab="meditation" label={t.meditationManagement} icon={<UserIcon />} />}
