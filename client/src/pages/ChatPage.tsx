...
// FIX: Changed to a named import as ConversationSidebar is a named export.
import { ConversationSidebar } from '../components/ConversationSidebar';
import MessageContextMenu from '../components/MessageContextMenu';
import PricingModal from '../components/PricingModal';
import { useToast } from '../components/ToastProvider';
// FIX: Import `SoundWaveIcon`
import { ChevronDownIcon, CopyIcon, SpeakerWaveIcon, ThumbsUpIcon, ThumbsDownIcon, ChevronDoubleRightIcon, CommentIcon, DownloadIcon, SoundWaveIcon, ShareIcon, PaperclipIcon } from '../components/Icons';
// FIX: Imported CryptoMeritPaymentModal instead of the empty CryptoPaymentModal file.
import CryptoMeritPaymentModal from '../components/CryptoMeritPaymentModal';
...
export const ChatPage: React.FC<{
  ...
  // FIX: Changed user.coins to user.merits to align with the User type definition.
  const isUserCoinsDepleted = !!user && user.merits !== null && user.merits <= 0 && !hasActiveSubscription;
  ...
  const aiToDisplay = currentAiConfig;
  const displayName = (language === 'en' && aiToDisplay?.nameEn) ? aiToDisplay.nameEn : aiToDisplay?.name;
  const displayDescription = (language === 'en' && aiToDisplay?.descriptionEn) ? aiToDisplay.descriptionEn : aiToDisplay?.description;
  
  const suggestedQuestionsRaw = (language === 'en' && aiToDisplay?.suggestedQuestionsEn) 
    ? aiToDisplay.suggestedQuestionsEn 
    : aiToDisplay?.suggestedQuestions;
  const displaySuggestedQuestions = normalizePostgresArray(suggestedQuestionsRaw);
  ...
  return (
    <div className={`chat-layout-container ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <ConversationSidebar 
        ...
        onOpenMeritPurchase={() => setIsCoinPurchaseModalOpen(true)} 
      ...
      />
            ...
            {viewMode === 'chat' ? (
                <>
                    <header className="chat-main-header relative z-20">
                        <div>{/* Left spacer for grid layout */}</div>

                        <div className="flex items-center justify-center gap-x-6 sm:gap-x-8">
                            <a href="https://giac.ngo/landing" target="_blank" rel="noopener noreferrer" className="text-base font-medium text-text-light hover:text-primary transition-colors">{t.home}</a>
                            <a href="https://giac.ngo/sutra" target="_blank" rel="noopener noreferrer" className="text-base font-medium text-text-light hover:text-primary transition-colors">{t.about}</a>
                            <a href="https://giac.ngo/library" target="_blank" rel="noopener noreferrer" className="text-base font-medium text-text-light hover:text-primary transition-colors">{t.library}</a>
                        </div>

                        <div className="flex items-center justify-end gap-4">
                            <div className="relative" ref={aiSelectorRef}>
                                <button onClick={() => setIsAiSelectorOpen(prev => !prev)} className="flex items-center gap-3 p-1.5 rounded-lg hover:bg-background-light border border-text-main">
                                    {aiToDisplay?.avatarUrl && <img src={aiToDisplay.avatarUrl} alt={displayName} className="w-8 h-8 rounded-full object-cover"/>}
                    ...
                    <div className="chat-messages-container" onClick={handleCloseContextMenu}>
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center px-4 relative z-10">
                            ...
                            />
                            <h2 className={`text-3xl font-bold text-text-main`}>{displayName}</h2>
                            <p className={`mt-2 text-lg text-text-light max-w-xl`}>{displayDescription}</p>
                            
                            {displaySuggestedQuestions && displaySuggestedQuestions.length > 0 && (
                                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl">
                                    {displaySuggestedQuestions.map((q, i) => (
                                        <button 
                                            key={i} 
                                    ...
                                    <div className={`chat-message-bubble ${msg.sender}`}>
                                        {msg.sender === 'ai' && aiToDisplay && (
                                            <div className="ai-message-sender">
                                                <img src={aiToDisplay.avatarUrl} alt={displayName} className="w-6 h-6 rounded-full object-cover" />
                                ...
                                {fileAttachment && (
                                     <div className="image-preview-container">
                                        <div className="file-preview-thumb">
                                            <PaperclipIcon className="w-8 h-8 text-text-light" />
                                            <span className="text-xs text-text-light truncate">{fileAttachment.name}</span>
                                        </div>
                                        <button type="button" onClick={() => setFileAttachment(null)} className="image-preview-remove-btn">&times;</button>
                                    </div>
                                )}
        ...
        <PricingModal isOpen={isPricingModalOpen} onClose={() => setIsPricingModalOpen(false)} user={user} onUserUpdate={onUserUpdate} language={language} />
        {user && <CryptoMeritPaymentModal isOpen={isCoinPurchaseModalOpen} onClose={() => setIsCoinPurchaseModalOpen(false)} user={user} onPaymentSuccess={onUserUpdate} language={language} />}
    </div>
  );
};