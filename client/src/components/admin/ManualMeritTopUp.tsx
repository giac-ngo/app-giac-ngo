// client/src/components/admin/ManualMeritTopUp.tsx
import React, { useState, useEffect } from 'react';
import { User } from '../../types';
import { apiService } from '../../services/apiService';
import { useToast } from '../ToastProvider';

interface ManualMeritTopUpProps {
    adminUser: User;
    language: 'vi' | 'en';
...
const translations = {
    vi: {
        ...
        feedback: {
            ...
            missingInfo: 'Vui lòng chọn người dùng và nhập số merit.',
            invalidData: 'Dữ liệu không hợp lệ.',
            success: 'Nạp thành công {amount} merit cho {name}. Số dư mới: {balance}.',
            failure: 'Nạp thất bại: {message}',
        }
    },
    en: {
        title: 'Manual Merit Top-Up',
        loading: 'Loading...',
        selectUserLabel: 'Select user to top-up',
        selectUserPlaceholder: '-- Select a user --',
        amountLabel: 'Amount of merits to add',
        ...
        feedback: {
            ...
            success: 'Successfully added {amount} merits for {name}. New balance: {balance}.',
            failure: 'Top-up failed: {message}',
        }
    }
};

const ManualMeritTopUp: React.FC<ManualMeritTopUpProps> = ({ adminUser, language, onTransactionSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const meritAmount = parseInt(amount, 10);

        if (isNaN(userId) || isNaN(meritAmount)) {
        ...
        try {
            const updatedUser = await apiService.addMeritsManually(userId, meritAmount, adminUser.id as number);
            const successMessage = t.feedback.success
                .replace('{amount}', String(meritAmount))
                .replace('{name}', updatedUser.name)
                .replace('{balance}', String(updatedUser.merits));
...
export default ManualMeritTopUp;