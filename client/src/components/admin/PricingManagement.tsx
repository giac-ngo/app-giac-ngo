...
const translations = {
    vi: {
        ...
        planName: 'Tên Gói (VI)',
        planNameEn: 'Tên Gói (EN)',
        price: 'Giá hiển thị (VI, VD: 30 Merits, Miễn phí)',
        priceEn: 'Giá hiển thị (EN, VD: 30 Merits, Free)',
        meritCost: 'Chi phí (Merits)',
        duration: 'Thời hạn (ngày)',
        durationPlaceholder: 'Để trống cho không giới hạn',
        features: 'Tính năng (VI, mỗi dòng một tính năng)',
        featuresEn: 'Tính năng (EN, mỗi dòng một tính năng)',
        isActive: 'Kích hoạt',
        save: 'Lưu',
        saving: 'Đang lưu...',
        cancel: 'Hủy',
    ...
    en: {
        ...
        editPlan: 'Edit Plan',
        planName: 'Plan Name (VI)',
        planNameEn: 'Plan Name (EN)',
        price: 'Display Price (VI, e.g., 30 Merits, Miễn phí)',
        priceEn: 'Display Price (EN, e.g., 30 Merits, Free)',
        meritCost: 'Cost (Merits)',
        duration: 'Duration (days)',
        durationPlaceholder: 'Leave blank for unlimited',
        features: 'Features (VI, one per line)',
...
const PricingManagement: React.FC<PricingManagementProps> = ({ language }) => {
    ...
    const handleNew = () => setEditingPlan({ id: 'new', planName: '', planNameEn: '', price: '', priceEn: '', meritCost: 0, durationDays: 30, features: [], featuresEn: [], isActive: true });
    ...
    const handleSave = async () => {
        ...
        try {
            const planToSave = {
                ...editingPlan,
                meritCost: Number(editingPlan.meritCost || 0),
                durationDays: editingPlan.durationDays ? Number(editingPlan.durationDays) : null,
                features: Array.isArray(editingPlan.features) ? editingPlan.features.filter(f => f.trim() !== '') : [],
                featuresEn: Array.isArray(editingPlan.featuresEn) ? editingPlan.featuresEn.filter(f => f.trim() !== '') : [],
            };
    ...
    const handleFormChange = (field: keyof PricingPlan, value: any) => {
        if (editingPlan) {
            setEditingPlan({ 
                ...editingPlan, 
                [field]: (field === 'features' || field === 'featuresEn') ? value.split('\n') : value 
            });
        }
    };
    ...
    const renderPlanForm = () => {
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div className="bg-background-panel rounded-lg shadow-xl w-full max-w-lg p-6 space-y-4 overflow-y-auto max-h-[90vh]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label className="block text-sm font-medium">{t.planNameEn}</label><input type="text" value={editingPlan.planNameEn || ''} onChange={e => handleFormChange('planNameEn', e.target.value)} className="mt-1 w-full p-2 border rounded" /></div>
                        ...
                        <div><label className="block text-sm font-medium">{t.priceEn}</label><input type="text" value={editingPlan.priceEn || ''} onChange={e => handleFormChange('priceEn', e.target.value)} className="mt-1 w-full p-2 border rounded" /></div>
                        <div><label className="block text-sm font-medium">{t.meritCost}</label><input type="number" value={editingPlan.meritCost || 0} onChange={e => handleFormChange('meritCost', e.target.value)} className="mt-1 w-full p-2 border rounded" /></div>
                    ...
                    <div><label className="block text-sm font-medium">{t.features}</label><textarea value={Array.isArray(editingPlan.features) ? editingPlan.features.join('\n') : ''} onChange={e => handleFormChange('features', e.target.value)} rows={4} className="mt-1 w-full p-2 border rounded"></textarea></div>
                    <div><label className="block text-sm font-medium">{t.featuresEn}</label><textarea value={Array.isArray(editingPlan.featuresEn) ? editingPlan.featuresEn.join('\n') : ''} onChange={e => handleFormChange('featuresEn', e.target.value)} rows={4} className="mt-1 w-full p-2 border rounded"></textarea></div>
                    ...
                    <table className="min-w-full divide-y divide-border-color">
                        <thead className="bg-background-light">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-text-light uppercase">{t.meritCost}</th>
                        ...
                        <tbody className="bg-background-panel divide-y divide-border-color">
                            {plans.map(plan => (
                                <tr key={plan.id}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{plan.planName} / {plan.planNameEn}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">{plan.price} / {plan.priceEn}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">{plan.meritCost}</td>
