...
import { ExpandIcon, PaperclipIcon } from '../Icons';
...
const translations = {
    ...
    en: {
        ...
        accountExpiredTitle: 'Your account is out of merits.',
        accountExpiredBodyTestChat: 'Please top up your merits to use the test chat window.',
...
const AiManagement: React.FC<{ language: 'vi' | 'en', user: User }> = ({ language, user }) => {
    ...
    const isUserAccountExpired = user ? (user.merits !== null && user.merits <= 0 && !hasActiveSubscription) : false;
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter AI list to show only user-owned AIs, unless user is an admin
                const userVisibleAIs = user.isAdmin ? configs : configs.filter(ai => ai.ownerId === user.id);
                setAiList(userVisibleAIs);
                setAllUsers(users);

                if (userVisibleAIs.length > 0) {
                    setSelectedAi(userVisibleAIs[0]);
                } else {
                    setSelectedAi(null);
                }
            } catch (error) {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (name === 'suggestedQuestions' || name === 'suggestedQuestionsEn') {
            const questions = value.split('\n');
            if (questions.length > 4) {
                showToast(t.suggestedQuestionsLimit, 'error');
                return; 
            }
            setSelectedAi(prev => prev ? { ...prev, [name]: questions.filter(q => q.trim() !== '') } : null);
        } else {
                ...
                <div className="flex-1 flex">
                    <div className={`p-6 border-r border-border-color overflow-y-auto relative flex flex-col transition-all duration-300 ease-in-out ${isChatExpanded ? 'w-1/3' : 'w-3/5'}`}>
                        ...
                        <div className="flex-grow space-y-6">
                            ...
                            <div>
                              ...
                              <div>
                                <div className="flex items-center justify-between">
                                    ...
                                    <button onClick={() => setIsQaModalOpen(true)} title={t.expand} className="p-1 text-text-light hover:text-primary">
                                    ...
                                    {qaTrainingData.length > 0 ? (
                                        qaTrainingData.map(item => (
                                            <div key={item.id} className="group relative p-2 bg-background-light rounded-md text-sm">
                                                ...
                                                {!isFormDisabled && <button onClick={() => handleDeleteTrainingData(item.id)} disabled={typeof item.id === 'number' && deletingIds.has(item.id)} className="absolute top-1 right-1 p-0.5 rounded-full bg-gray-300 text-white opacity-0 group-hover:opacity-100 hover:bg-accent-red disabled:opacity-50">&times;</button>}
                                            </div>
                                        ))
                                    ) : (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t border-border-color">
                                {/* Left Column */}
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-1">
                                            ...
                                            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading || isFormDisabled} className="p-1.5 rounded-md hover:bg-background-light text-text-light transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title={t.attachFile}>
                                                {isUploading ? <div className="w-5 h-5 border-2 border-dashed rounded-full animate-spin border-primary"></div> : <PaperclipIcon className="w-5 h-5" />}
                                            </button>
                                        </div>
                                        <div className="border border-border-color rounded-lg p-2 h-40 flex flex-col mt-1">
                                            <div className="flex-grow min-h-0 overflow-y-auto space-y-1 pr-1">
                                                {stagedFiles.map((file, index) => (
                                                    <div key={index} className="flex justify-between items-center bg-blue-50 border border-blue-200 px-2 py-1 rounded-md text-sm">
                                                ...
                                                {fileTrainingData.map((item) => (
                                                    <div key={item.id} className="flex justify-between items-center bg-background-light px-2 py-1 rounded-md text-sm">
                                                        <a href={item.fileUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline truncate flex-1" title={item.fileName}>{item.fileName}</a>
                                                        {!isFormDisabled && <button onClick={() => handleDeleteTrainingData(item.id)} disabled={typeof item.id === 'number' && deletingIds.has(item.id)} className="ml-2 text-accent-red hover:text-accent-red-hover flex-shrink-0 text-lg leading-none disabled:opacity-50">&times;</button>}
                                                    </div>
                                                ))}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-4 pt-2">
                                        ...
                                        <div>
                                            <label className="flex items-center"><input type="checkbox" name="requiresSubscription" checked={selectedAi.requiresSubscription} onChange={handleInputChange} disabled={isFormDisabled} className="w-4 h-4 text-primary bg-background-panel border-border-color rounded focus:ring-primary disabled:cursor-not-allowed" /><span className="ml-3 text-sm font-medium text-text-main">{t.requiresSub}</span></label>
                                    ...
                                    <div className="flex justify-end space-x-3 border-t border-border-color pt-4 mt-6">
                                        <button onClick={handleInitiateDelete} disabled={isFormDisabled || isSaving || typeof selectedAi.id !== 'number'} className="px-4 py-2 text-sm font-medium text-text-on-primary bg-accent-red rounded-md hover:bg-accent-red-hover disabled:opacity-50 disabled:cursor-not-allowed">{t.delete}</button>
                                        <button onClick={handleSaveChanges} disabled={isFormDisabled || isSaving} className="px-4 py-2 text-sm font-medium text-text-on-primary bg-primary rounded-md hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed">{isSaving ? t.saving : t.save}</button>
                                    </div>
                                </div>
                            </div>                        
                        </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                {messages.map((msg, index) => (
                                    <div key={msg.id} className={`group flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div className={`max-w-md px-4 py-2 rounded-xl ${msg.sender === 'user' ? 'bg-primary text-text-on-primary' : 'bg-background-light shadow-sm'}`}>
                                            {msg.text}
                                        </div>
                                        {msg.sender === 'ai' && !isFormDisabled && typeof selectedAi.id === 'number' && (
                                            <button 
                                    ...
                                    {qaTrainingData.length > 0 ? (
                                        qaTrainingData.map(item => (
                                            <div key={item.id} className="group relative p-3 bg-background-light rounded-lg text-sm border border-border-color">
                                                ...
                                                <p><strong className="font-semibold text-primary-text">{t.answer}:</strong> {item.answer}</p>
                                                {!isFormDisabled && <button onClick={() => handleDeleteTrainingData(item.id)} disabled={typeof item.id === 'number' && deletingIds.has(item.id)} className="absolute top-2 right-2 p-1 rounded-full bg-gray-300 text-white opacity-0 group-hover:opacity-100 hover:bg-accent-red leading-none disabled:opacity-50">&times;</button>}
                                            </div>
                                        ))
                                    ) : (
